...
#
# The fast-and-furious tab is a variant of simple-motion: SAME super-realistic
# photographic scenes (it reuses ``REALISTIC_STYLE`` images), SAME one-8s-video /
# two-shot geometry — but the voiceover is TikTok-style: lively, fast, modern,
# Gen Z. It is written to FILL the whole video (the row processor drives the video
# length from the voiceover, so there is no silent tail), which is why the word
# budget is larger than simple-motion's. Because these run on PAID native
# (Taboola/Outbrain) over sensitive verticals (weight-loss, finance, property),
# the punchy tone is fenced by an explicit ad-compliance block — energy comes from
# delivery + real detail, never from crossing a compliance line. Same JSON shape,
# same placeholders, same hard visual rules. Plan
...
# fast-and-furious reuses the simple-motion pipeline (same 1-video / 2-shot work),
# so it gets the same headroom under its own key.
...
SETTINGS_REGISTRY: tuple[SettingDef, ...] = (
    ...
    SettingDef(
        ...
        description=(
            ...
            "Gen-Z voiceover written to fill the whole video (no silent tail). "
            "Fenced by an ad-compliance block for paid native. Same {language}, "
            "{num_ideas}, {num_shots}, {target_words}, {min_words}, {max_words} "
            "placeholders. Pasted images (columns D/E) are animated as-is."
        ),
    ),
    SettingDef(
    ...
    SettingDef(
        ...
        multiline=False,
        description=(
            "Hard wall-clock budget for a fast-and-furious-tab row. It can "
            "produce up to 4 realistic videos per row (each = image-gen + 2 "
            "Seedance clips + TTS/caption), which queue on the shared keys — so "
            "it gets the largest headroom. Env "
