...
For EACH idea return:
- voiceover: a HIGH-ENERGY spoken script in {language}, about {target_words} words ({min_words}-{max_words}), written as 2-4 SHORT, snappy sentences that hit fast — the kind of hyped, lively, Gen-Z TikTok voiceover that grabs you and does NOT let go. It should sound genuinely EXCITED, like a young creator who cannot wait to tell you this. Front-load the punch: OPEN WITH A SCROLL-STOPPING HOOK in the first 2-4 words — a direct "you", a bold question, or one surprising-but-true detail from the article. Keep sentences short and the momentum relentless; favour vivid, punchy, everyday words and rhetorical questions over flat description. Use enough words to fill the whole ~8 seconds; do NOT trail off or leave dead air. Write it in NATURAL, current, spoken {language} — match how young people in that market actually talk (do not force English slang into another language). The script MUST end on a COMPLETE THOUGHT — ideally an exclamation — a period, question mark, or exclamation mark. NEVER end on a conjunction (and, but, or, so, because, with, that, which, as) or a preposition. End on a strong, punchy word that lands hard.
- style_direction: a vivid delivery hint pushing MAXIMUM energy — fast, hyped, upbeat, punchy, excited, conversational, like a young TikTok creator talking straight to camera. Explicitly call for a fast pace and rising, enthusiastic intonation; NEVER calm, flat, slow, or neutral.
- shots: an array of exactly {num_shots} shots, each with:
    * scene: a vivid description of ONE realistic, photographic scene (real-looking person or setting, subject, framing, lighting). Vertical composition. Describe it as a real photo or live footage — NOT a cartoon, illustration, or animation.
